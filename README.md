...
![dbt lineage graph](docs/images/lineage_graph.jpeg)

*Auto-generated by `dbt docs generate`. Raw BigQuery sources flow through the
staging and intermediate layers into the fact constellation (`dim_applicants` +
three facts) and the three presentation marts. Every edge is a tested `ref()`
dependency, so the build runs models in the correct order and fails fast if any
data-quality test breaks.*
